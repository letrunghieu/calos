...
Route::filter('after', function($response)
	{
		...
		\Navigation\Navigation::make('topbar')
			->add_link(__('activity.activity'), URL::to_action('activity'), false, array(), null, 'activities_menu')
			->add_link(__('organization.organization'), '', false, array(
				...
				, 'organization_menu')
			->add_link(__('announcement.announcements'), URL::to_action('announcement'), false, array(), null, 'announcements_menu')
		...
		if (\CALOS\Services\UserService::can_write_announcement($current_user->id))
		{
		    \Navigation\Navigation::get('topbar')->find_item('tools_menu')
			    ->get_child_or_create()
			    ->add_link("" . __('announcement.write new announcement'), URL::to_action('announcement@create'))
		    ;
		}
